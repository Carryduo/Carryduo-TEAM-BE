...
export class RequestSummonerRiotAPI {
  ...
  async requestRiotSummonerApi() {
    ...
    const detailResponse = await axios.get(
      `https://kr.api.riotgames.com/lol/league/v4/entries/by-summoner/${summonerDataResult.id}?api_key=${process.env.RIOT_API_KEY}`,
    );
    ...
    const mostChampResponse = await axios.get(
      `https://kr.api.riotgames.com/lol/champion-mastery/v4/champion-masteries/by-summoner/${summonerDataResult.id}/top?count=3&api_key=${process.env.RIOT_API_KEY}`,
    );
