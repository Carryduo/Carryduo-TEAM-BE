...
export class ChampService {
  constructor(
    ...
    private readonly transfer: TransferChampData,
  ) {}
  ...
  async getTargetChampion(param: TargetChampionReqDTO) {
    ...
    const champData = await this.champRepository.getChampDefaultData(
      param.champId,
    );
    ...
    const champPosition = await this.transfer.champPosition(
      param,
      rateLatestVersion,
    );
    const banInfo = await this.champRepository.getBanRate(
      param.champId,
      rateLatestVersion,
    );
    const champRateInfo: GetChampRate[] | [] =
      await this.champRepository.getChampRate(
        param.champId,
        champPosition,
        rateLatestVersion,
      );
    const champRate = await this.transfer.champRate(
      champRateInfo,
      banInfo?.banRate,
      champPosition,
    );
