...
describe('CommentsService', () => {
  ...
  it('postComment 가 잘 실행되나?', async () => {
    ...
    repository.createSelectOption = jest
      .fn()
      .mockImplementation((requestOption) => {
        return requestOption;
      });
    repository.postComment = jest.fn().mockImplementation((requestOption) => {
      return requestOption;
    });
    repository.setCommentCache = jest
      .fn()
      .mockImplementation((category, target, requestOption) => {
        return;
      });
    const parameter = new PostCommentRequestDto(
      user.userId,
      category,
      target,
      body.content,
    ...
    expect(repository.setCommentCache).toBeCalledWith(
      parameter.category,
      parameter.target,
      parameter.toEntity(),
    );
  ...
  it('updateComment가 잘 실행되나?', async () => {
    ...
    try {
      await service.updateContent(
        new UpdateCommentRequestDto(null, null, 'asd'),
      );
    } catch (err) {
      console.log(err);
  ...
  it('updateReportNum이 잘 실행되나?', async () => {
    ...
    repository.updateReportNum = jest
      .fn()
      .mockImplementation((requestOption) => {
        return comments.champ;
      });
